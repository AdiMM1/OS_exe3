...
void handleCommand(const string& command, int client_socket) {
    try {
        if (command.find("Newgraph") == 0) {
            ...
            for (int i=0; i<m; ++i){
                ...
                if (bytes_read > 0){
                    ...
                    string error = "Error: Failed to read edge.\n";
            ...
            if (g != nullptr){
                ...
                string error = "Error: No graph exists. Use 'Newgraph' to create one.\n";
                send (client_socket, error.c_str(), error.size(), 0);
            }    
        ...
        string error = "Error: Exception occurred: ";
        error += e.what();
        error += "\n";
        send(client_socket, error.c_str(), error.size(), 0);
    } catch (...) {
        string error = "Error: Unknown exception occurred.\n";
        send(client_socket, error.c_str(), error.size(), 0);
    }
}
